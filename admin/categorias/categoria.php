    ...
    <div class="col-xs-12 col-md-12">
      <form action="salvar_categoria.php" method="POST">
        ...
        <div class="form-group">
          <label>Nome</label>
          <input type="text" value="<?php  if(isset($row[0])){echo $row[0]; } ?>" class="form-control" name="nome" />
        </div>
        <div class="form-group">
          <label>Descrição</label>
          <input type="text" value="<?php if(isset($row[1])) { echo $row[1]; } ?>" class="form-control" name="descricao" />
        </div>
